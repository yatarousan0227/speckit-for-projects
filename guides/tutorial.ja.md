...
補足として、リポジトリには `src/speckit_for_projects/templates/project/design-system.md.j2` と `src/speckit_for_projects/templates/project/ui-storybook/` もあります。ただし `shadcn/ui` のような外部デザインシステム採用案件では不要になりうるため、現行 `sdd init` はそれらを自動配置しません。このチュートリアルでも必須前提にはしていません。
