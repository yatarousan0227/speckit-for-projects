...
この文書は、`SpecKit for Projects` を初めて使う人向けの日本語チュートリアルです。空のリポジトリに `SpecKit for Projects` を導入し、`brief -> clarify -> design -> tasks -> analyze -> implement` を 1 回通したうえで、必要に応じて `debug` と `reflect` を使うところまでを扱います。
...
Codex では `.codex/prompts/*.md` は slash command にはなりません。`--ai-skills` を付けておくと、Codex から見つけやすい `speckit-for-projects-analyze`、`speckit-for-projects-brief`、`speckit-for-projects-clarify`、`speckit-for-projects-debug`、`speckit-for-projects-reflect` などの skill も併せて入ります。
...
迷った場合は、[examples/project-standards/todo-app-ja/README.md](../examples/project-standards/todo-app-ja/README.md) を参考にしてください。
...
参考例は [examples/screen-centric/briefs/001-screened-application-portal.md](../examples/screen-centric/briefs/001-screened-application-portal.md) です。
...
- [examples/screen-centric/designs/specific_design/001-screened-application-portal/overview.md](../examples/screen-centric/designs/specific_design/001-screened-application-portal/overview.md)
...
- [examples/screen-centric/designs/specific_design/001-screened-application-portal/tasks.md](../examples/screen-centric/designs/specific_design/001-screened-application-portal/tasks.md)
...
予定どおりに task を進めるなら `sdd.implement` で足りますが、想定外の不具合修正や手動差分の追随では使い分けます。

- 不具合の再現確認、コード修正、検証、設計書同期まで一連で進めたいなら `sdd.debug`
- 開発者がすでに入れた current working tree diff を正として、設計書と `tasks.md` を追随させたいなら `sdd.reflect`
- どちらも `briefs/*.md` は更新しません
